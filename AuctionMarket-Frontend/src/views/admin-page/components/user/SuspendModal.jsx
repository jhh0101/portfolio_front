...
const SuspendModal = ({ isOpen, onClose, userId }) => {
    ...
    const bidCount = userStatus?.data?.bidCount || 0;
    const productCount = userStatus?.data?.productCount || 0;
    const isReadyToDelete = isChecked && suspensionReason.length > 0;

    if (!isOpen) return null;

    const handleSubmit = async (e) => {
        e.preventDefault();
        const suspensionReason = new FormData(e.currentTarget).get("suspensionReason");
        if(!window.confirm("정말 회원을 정지하시겠습니까?")) return;
        await suspend({suspensionReason});
        onClose();
    };
            ...
            <div className="modal-content" style={{width: "400px"}} onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="modal-body">
                        Nick Name : <h5 style={{display: "inline"}}>{userStatus.data?.nickname}님</h5>
